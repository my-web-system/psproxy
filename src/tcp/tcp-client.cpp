...
void TcpClient::Start() {
    ...
    if (status < 0) {
        ...
        PSP_DBG("psp connected to client " + _host + ":" + std::to_string(_port))
    }
}
...
std::shared_ptr<Packet<HTTPResponse>> TcpClient::Forward(const std::shared_ptr<Packet<HTTPRequest>> &packet) {
    ...
    std::shared_ptr<Packet<HTTPResponse>> packetResponse(new Packet<HTTPResponse>(packet->getServerSocket(), httpResponse));
    return packetResponse;
}

TcpClient::~TcpClient()
{
    std::cout << "Close client " << _socket_fd << std::endl;
